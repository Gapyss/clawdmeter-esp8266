...
- **Watch dashboard page growth.** If `INDEX_HTML` (gzipped) approaches the `hb2f` send buffer again, the one-buffer assumption breaks; keep `gen_index_gz.py`'s reported gz size in view. (Owner: Gapyss.)
